-- AstralDB torture harness (SQLite-shaped names; supported grammar only).
-- Uses INSERT ... BULK for dense fixtures instead of megabyte-long VALUES lists.
-- Stresses lexer, WAL, FILTER_DNF, UPDATE/DELETE, DISTINCT/ORDER BY/LIMIT.
...
INSERT INTO torture_test BULK 1000 START 1 STEP 1;

-- Narrow updates (predicate fan-out via OR + AND + BETWEEN)
UPDATE torture_test SET c = 'hot' WHERE id > 900 AND id < 1100;
...
UPDATE torture_test SET b = 'upd2' WHERE id BETWEEN 980 AND 995 AND a >= 10 AND a <= 990;
...
DELETE FROM torture_test WHERE id BETWEEN 1150 AND 1250;
