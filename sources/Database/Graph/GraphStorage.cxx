...
void RegisterGraphCatalogEntryAssumeLocked(Database &Db, GraphSpec Spec) {
	...
	GraphAdjacency Adj;
	const HybridTableSlot &EdgeSlot = Db.Tables_.at(Spec.EdgeTable);
	if(EdgeSlot.Columnar.BulkSyntheticLazy && EdgeSlot.Columnar.RowCount > 0) {
		Spec.LazyBulkEdges = true;
	} else {
		const Database::Table &EdgeRows = EdgeSlot.RowsForRead(std::nullopt, false);
...
bool StripAndParseGraphCatalogSnapshotTrailer(std::string &RawData, std::vector<GraphSpec> &OutSpecs) {
	OutSpecs.clear();
	const size_t Mp = RawData.rfind(kGraphSnapshotMarkerSv);
