...
std::optional<std::string> NfpMacroStepCellFromReal(const std::string &G, const std::string &Grid,
                                                      ...
                                                      const std::string &Dt) {
	const auto Gv = ParseRealList(G);
	const auto Gridv = ParseRealList(Grid);
	const auto Driftv = ParseRealList(Drift);
	const auto Diffv = ParseRealList(Diffusion);
	const auto Neuv = ParseRealList(NeuralCorr);
