...
void PatchBranch19(std::vector<std::uint8_t> &Out, std::size_t At, std::int32_t ByteOffset) {

	const std::int32_t Imm19 = ByteOffset >> 2;

	std::uint32_t Insn = 0;

	Insn |= static_cast<std::uint8_t>(Out[At + 0]);
...
bool EmitFilterDense(std::vector<std::uint8_t> &Out, FilterCompareOp Op) {
	...
	EmitU32(Out, 0xAA1F03E5u); // mov x5, xzr (index)

	const std::size_t CbzAt = Out.size();

	EmitU32(Out, 0xB4000001u); // cbz x1, end
	...
	EmitU32(Out, 0xF8296825u); // str x5, [x3, x4, lsl #3]
	...
	EmitU32(Out, 0x54000001u); // b.ne loop
	...
	PatchBranch19(Out, CbzAt, static_cast<std::int32_t>(End - CbzAt));
...
bool EmitSum(std::vector<std::uint8_t> &Out) {
	...
	EmitU32(Out, 0xB4000001u); // cbz x1, done
	...
	PatchBranch19(Out, CbzAt, static_cast<std::int32_t>(Done - CbzAt));
...
bool EmitMin(std::vector<std::uint8_t> &Out) {
	...
	EmitU32(Out, 0x5400000Bu); // b.le skip
...
bool EmitMax(std::vector<std::uint8_t> &Out) {
	...
	EmitU32(Out, 0x5400000Cu); // b.ge skip
